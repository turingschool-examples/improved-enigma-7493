...
RSpec.describe Chef do
  ...
  it "has a page with a link" do
    visit "/chefs/#{@chef1.id}"

    expect(page).to have_content("Chef #{@chef1.name}'s Page")
    expect(page).to have_link("The Secret Ingredients")
    click_link("The Secret Ingredients")
